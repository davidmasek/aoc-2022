package main

import (
	"fmt"
	"log"
	"os"
	"strconv"
	"strings"
)

type Point struct {
	X, Y int
}

const (
	Wall = iota
	Space
	Ground
)

const (
	Up    = "Up"
	Left  = "Left"
	Right = "Right"
	Down  = "Down"
)

type Node struct {
	X, Y      int
	Neighbors map[string]*Node
	Type      int
}

func parseInput(input string) map[Point]*Node {
	nodes := make(map[Point]*Node, 0)
	lines := strings.Split(input, "\n")
	width := len(lines[0])
	height := len(lines)

	for y, line := range lines {
		for x, char := range line {
			if char == '.' {
				nodes[Point{x, y}] = &Node{X: x, Y: y, Type: Ground, Neighbors: make(map[string]*Node)}
			} else if char == '#' {
				nodes[Point{x, y}] = &Node{X: x, Y: y, Type: Wall}
			} else if char == '+' {
				nodes[Point{x, y}] = &Node{X: x, Y: y, Type: Space}
			} else {
				panic("unexpected char " + string(char))
			}
		}
	}

	for _, node := range nodes {
		if node.Type != Ground {
			continue
		}
		i := 1
		for {
			x := (node.X + i + width) % width
			if nb, found := nodes[Point{x, node.Y}]; found {
				if nb.Type == Wall {
					node.Neighbors[Right] = nil
					break
				}
				if nb.Type == Ground {
					node.Neighbors[Right] = nb
					break
				}
			}
			i++
		}

		i = -1
		for {
			x := (node.X + i + width) % width
			if nb, found := nodes[Point{x, node.Y}]; found {
				if nb.Type == Wall {
					node.Neighbors[Left] = nil
					break
				}
				if nb.Type == Ground {
					node.Neighbors[Left] = nb
					break
				}
			}
			i--
		}

		i = -1
		for {
			y := (node.Y + i + height) % height
			if nb, found := nodes[Point{node.X, y}]; found {
				if nb.Type == Wall {
					node.Neighbors[Up] = nil
					break
				}
				if nb.Type == Ground {
					node.Neighbors[Up] = nb
					break
				}
			}
			i--
		}

		i = 1
		for {
			y := (node.Y + i + height) % height
			if nb, found := nodes[Point{node.X, y}]; found {
				if nb.Type == Wall {
					node.Neighbors[Down] = nil
					break
				}
				if nb.Type == Ground {
					node.Neighbors[Down] = nb
					break
				}
			}
			i++
		}
	}

	return nodes
}

type Move struct {
	dir  string
	size int
}

func MakeMoves(graph map[Point]*Node, path []Move) {
	var start *Node
	for _, n := range graph {
		if n.Type == Ground {
			if start == nil {
				start = n
			} else if n.Y < start.Y || (n.Y == start.Y && n.X < start.X) {
				start = n
			}
		}
	}
	fmt.Println("start:", start)

	current := start
	for _, move := range path {
		for i := 0; i < move.size; i++ {
			nb := current.Neighbors[move.dir]
			if nb != nil {
				current = nb
			}
		}
	}
	fmt.Println("end:", current)
	var facing int
	switch path[len(path)-1].dir {
	case Up:
		facing = 3
	case Left:
		facing = 2
	case Right:
		facing = 0
	case Down:
		facing = 1
	default:
		panic("aaaa")

	}
	lastMove := path[len(path)-1]
	fmt.Println(lastMove, facing)
	pwd := 1000*current.Y + 4*current.X + facing
	fmt.Println("A:", pwd)
}

func main() {
	content, err := os.ReadFile("in.txt") // Replace with your file name
	if err != nil {
		log.Fatal(err)
	}
	// pathStr := "10R5L5R10L4R5L5"
	pathStr := ""
	pathStr = strings.ReplaceAll(pathStr, "R", ",R;")
	pathStr = strings.ReplaceAll(pathStr, "L", ",L;")

	dirs := []string{
		"Up", "Right", "Down", "Left",
	}
	currentDirIndex := 1
	currentDir := dirs[currentDirIndex]
	moves := make([]Move, 0)

	for _, pair := range strings.Split(pathStr, ";") {
		out := strings.Split(pair, ",")
		if len(out) == 2 {
			left, right := out[0], out[1]
			leftInt, err := strconv.Atoi(left)
			if err != nil {
				panic("aaaa")
			}
			move := Move{currentDir, leftInt}
			moves = append(moves, move)
			if right == "L" {
				currentDirIndex = (currentDirIndex - 1 + 4) % 4
			} else if right == "R" {
				currentDirIndex = (currentDirIndex + 1 + 4) % 4
			} else {
				panic("aaaa")
			}
			currentDir = dirs[currentDirIndex]
		} else if len(out) == 1 {
			left := out[0]
			leftInt, err := strconv.Atoi(left)
			if err != nil {
				panic("aaaa")
			}
			move := Move{currentDir, leftInt}
			moves = append(moves, move)
		} else {
			panic("AAAA")
		}
	}

	graph := parseInput(string(content))
	MakeMoves(graph, moves)
}
